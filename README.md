...
服务默认监听 `0.0.0.0:8480`，并在后台运行。首次执行会自动创建：
...
3. 在手机浏览器中打开显示的地址，例如 `http://192.168.1.100:8480`。

如果手机无法访问，请检查 Ubuntu 防火墙是否允许 TCP 端口 `8480`：

```bash
sudo ufw allow 8480/tcp
```
...
默认端口为 `8480`。可以通过环境变量临时指定其他监听地址或端口：

```bash
HOST=0.0.0.0 PORT=8480 ./run.sh start
```
...
新增共享美食时使用“风味 / 类型”列表，不要求所有选项都是传统菜系。例如面食可选择
汤面、米粉米线、饺子馄饨、饼类、馍类或地方小吃。标签和推荐理由均不需要填写。
